var Indodax = require('../indodax'),
  _ = require('lodash'),
  moment = require('moment'),
  log = require('../../core/log'),
  exchangeUtils = require('../exchangeUtils'),
  scientificToDecimal = exchangeUtils.scientificToDecimal;
...
const capabilities = {
  ...
  markets: [
    ...
    { pair: ['IDR', 'BTC'], minimalOrder: { amount: 0.0001, price: 10000, order: 0.0000001, unit: 'asset' } },
...
var Trader = function(config) {
  ...
  this.name = 'Indodax';
  this.indodax = new Indodax(this.key, this.secret);
  this.pair = this.asset + this.currency;
  this.market = _.find(Trader.getCapabilities().markets, (market) => {
    return market.pair[0].toLowerCase() === this.currency && market.pair[1].toLowerCase() === this.asset
  });
}
...
Trader.prototype.getTicker = function(callback) {
  ...
  this.indodax.getTicker(this.pair, set);
}
...
Trader.prototype.getPortfolio = function(callback) {
  ...
  set = function(err, data) {
    ...
    if(
      ...
      return log.error('account balance error: Gekko is unable to trade with ',this.currency,':',currencyAmount,' or ',this.asset,':',assetAmount);
    }
    var portfolio = [
  ...
  this.indodax.getAccountBalances(set);
}
...
Trader.prototype.getTrades = function(since, callback) {
  ...
  this.indodax.getTrades(this.pair, process);
}
...
Trader.prototype.checkOrder = function(order, callback) {
  ...
  this.indodax.getOrderDetails(this.pair, order, check);
}
...
Trader.prototype.getOrder = function(order, callback) {
  ...
  var get = function(data, err) {
   ...
   this.indodax.getOrderDetails(this.pair, order, get);
}
...
Trader.prototype.cancelOrder = function(order, callback) {
  ...
  this.indodax.cancelOrder(this.pair, order, this.type, cancel);
}
